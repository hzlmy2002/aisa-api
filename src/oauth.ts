...
export const authFile = (home = homedir()) => path.resolve(home, '.aisa/aisa-api/oauth.json');
...
export async function login(file: string, noBrowser = false, io = {input: process.stdin, output: process.stderr, fetcher: fetch, launch: openBrowser}) {
  ...
  try {
    ...
    const client = await request(metadata.registration_endpoint, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({client_name: 'AIsa API', redirect_uris: [redirect], grant_types: ['authorization_code', 'refresh_token'], response_types: ['code'], token_endpoint_auth_method: 'none', scope: 'profile email offline_access'})}, fetcher);

...
async function main() {
  const {values, positionals} = parseArgs({allowPositionals: true, options: {modules: {type: 'string'}, server: {type: 'string'}, 'all-tools': {type: 'boolean'}, 'discovery-tools': {type: 'boolean'}, version: {type: 'boolean'}, client: {type: 'string'}, home: {type: 'string'}, auth: {type: 'string'}, 'no-browser': {type: 'boolean'}, 'install-skills': {type: 'boolean'}, help: {type: 'boolean'}}});
  if (values.version) {console.log(VERSION); return;}
  const serverOptions = {modules: values.modules?.split(',').map(value => value.trim()), server: values.server?.trim(), allTools: values['all-tools'], discoveryTools: values['discovery-tools']};
  launchArgs(serverOptions);
  const command = positionals[0] ?? 'serve';
  if (values.help) {
    console.log('aisa-api serve [--modules comma,list] [--server slug] [--all-tools] [--discovery-tools] [--install-skills --client codex|claude-code|hermes]\naisa-api setup [--modules comma,list] [--server slug] [--all-tools] [--discovery-tools] [--client codex|claude-code|hermes] [--home directory] [--auth oauth|key] [--no-browser]\naisa-api uninstall [--client codex|claude-code|hermes] [--home directory]\naisa-api status\nDefault serve exposes search, get_details, use and batch_use; the aisa-api skill routes to local server indexes and operation details. --discovery-tools restores the legacy search and resource helper tools. Modules/server optionally pin the surface; --all-tools exposes direct tools. setup detects all supported clients in your home directory when --client is omitted. setup reuses existing credentials or starts OAuth automatically; type key during login to use an API Key. --auth forces a method. --no-browser prints the OAuth URL without opening it; paste the complete callback URL after login.'); return;
  }
